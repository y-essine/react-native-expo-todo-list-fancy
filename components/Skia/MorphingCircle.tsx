...
export const MorphingCircle = () => {
  ...
  const path = useDerivedValue(() => {
    animate();
    return spline(points.value, 1, true);
  }, [clock]);

  const colorNoise = useDerivedValue(() => {
    hueNoiseOffset.value += noiseStep / 2;
    const hueNoise = noise(hueNoiseOffset.value, hueNoiseOffset.value);
