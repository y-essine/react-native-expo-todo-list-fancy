...
export default function App() {
  ...
  const theme = darkMode ? colors.dark : colors.light;  const handleAddTodo = () => {
    addTodo(newTodo);
    setNewTodo('');
  };
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      ...
      >
        <LinearGradient
          ...
          style={{ flex: 1 }}
        >
          <StatusBar style={darkMode ? 'light' : 'dark'} backgroundColor="transparent" />          <ScrollView
            ref={scrollViewRef}
            style={{ flex: 1 }}
          ...
          <AnimatedCard>
            <BlurCard style={{ marginBottom: 20 }} darkMode={darkMode}>
              <View style={{ 
                ...
                <View style={{ flexDirection: 'row', flexShrink: 0 }}>
                  <TouchableOpacity
                    ...
                    style={{
                      padding: 12,
                      backgroundColor: showStats ? theme.primary : theme.filterBackground,
                      borderRadius: 12,
                      marginRight: 8,
                    }}
                  >
              ...
              <View style={{ flexDirection: 'row' }}>
                <View style={{ flex: 1, marginRight: 12 }}>                  <TextInput
                    value={newTodo}
            ...
            <AnimatedCard delay={200}>
              <BlurCard style={{ marginBottom: 20 }} darkMode={darkMode}>
                <Text style={{ fontSize: 18, fontWeight: '600', color: theme.text, marginBottom: 16 }}>
                  Statistics
                </Text>                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
                  {([
                    { label: 'Total', value: stats.total, variant: 'default' as const },
                    { label: 'Completed', value: stats.completed, variant: 'cool' as const },
                    { label: 'Pending', value: stats.pending, variant: 'warm' as const },
                    { label: 'High Priority', value: stats.highPriority, variant: 'electric' as const },
                  ]).map((stat, i) => (
                    <FancyCard
                      key={i}
                      darkMode={darkMode}
            ...
            <BlurCard style={{ marginBottom: 20 }} darkMode={darkMode}>
              <View style={{ marginBottom: 16 }}>
                <View style={{ position: 'relative' }}>
                  <TextInput
                    value={search}
                    onChangeText={setSearch}                    placeholder="Search tasks..."
                    placeholderTextColor={theme.textMuted}
                    style={{
                      backgroundColor: theme.inputBackground,
                      borderRadius: 12,
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View style={{ flexDirection: 'row', gap: 8 }}>                  {filters.map((f) => (
                    <TouchableOpacity
                      key={f}
                      ...
                      style={{
                        ...
                        paddingVertical: 8,
                        borderRadius: 12,
                        backgroundColor: filter === f ? theme.primary : theme.filterBackground,
                      }}
                    >
                      <Text
                        style={{
          ...
          </AnimatedCard>          {/* Todo List */}
          <View style={{ gap: 12 }}>            {filteredTodos.map((todo, index) => (
              <TodoItem
                key={todo.id}
                todo={todo}
                index={index}
          ...
          {filteredTodos.length === 0 && (
            <AnimatedCard delay={400}>
              <View style={{ alignItems: 'center', paddingVertical: 40 }}>
                ...
                <Text style={{ fontSize: 18, color: theme.textSecondary, marginBottom: 8 }}>
                  No tasks found
                </Text>
